...
public:
    CompiledAlgoValue(const std::string &_algo_name, Value *(*_fn)(),
        ...
        : Value(VALUE_ALGO), fn(_fn), algo_name(_algo_name),
...
std::shared_ptr<Value> call_compiled(CompiledAlgoValue *algo,
...
std::shared_ptr<Value> CompiledAlgoValue::execute(const NodeList &args,
                                                  SymbolTable *parent) {
...
Value *rt_make_int(int64_t v) {
    return track(std::make_shared<TypedValue<int64_t>>(VALUE_INT, v));
}
...
Value *rt_make_string(const char *s) {
    return track(
        std::make_shared<TypedValue<std::string>>(VALUE_STRING, std::string(s)));
}

Value *rt_make_none() { return track(std::make_shared<Value>()); }

Value *rt_array_new() {
    return track(std::make_shared<ArrayValue>(ValueList(0)));
}

void rt_array_push(Value *arr, Value *v) {
    dynamic_cast<ArrayValue *>(arr)->push_back(ref(v));
}
...
Value *rt_bin_op(int64_t op, Value *a, Value *b) {
    ...
    switch (op) {
    case RT_OP_ADD: return track(lhs + rhs);
    case RT_OP_SUB: return track(lhs - rhs);
    case RT_OP_MUL: return track(lhs * rhs);
    case RT_OP_DIV: return track(lhs / rhs);
    case RT_OP_MOD: return track(lhs % rhs);
    case RT_OP_POW: return track(pow(lhs, rhs));
    case RT_OP_EQUAL: return track(lhs == rhs);
    case RT_OP_NEQ: return track(lhs != rhs);
    case RT_OP_LESS: return track(lhs < rhs);
    case RT_OP_GREATER: return track(lhs > rhs);
    case RT_OP_LEQ: return track(lhs <= rhs);
    case RT_OP_GEQ: return track(lhs >= rhs);
    default:
...
Value *rt_unary_op(int64_t op, Value *a) {
    ...
    switch (op) {
    case RT_OP_UPLUS: return track(operand);
    case RT_OP_UNEG: return track(-operand);
    case RT_OP_UNOT: return track(!operand);
    default:
...
Value *rt_index(Value *obj, Value *idx) {
    ...
    if (container->get_type() == VALUE_STRING) {
        ...
        if (1 <= p && p <= static_cast<int>(str.size())) {
            return track(std::make_shared<TypedValue<std::string>>(
    ...
    if (container->get_type() != VALUE_ARRAY) {
        return track(std::make_shared<ErrorValue>(
            VALUE_ERROR,
            "Access can only apply on array, find " + container->get_type() + "\n"));
    }
    return track(
        dynamic_cast<ArrayValue *>(container.get())->operator[](index->as_int()));
}
...
Value *rt_index_assign(Value *obj, Value *idx, Value *v) {
    ...
    if (container->get_type() == VALUE_HASH_TABLE) {
        return track(
            dynamic_cast<HashTableValue *>(container.get())->set(index, value));
    }
    if (container->get_type() != VALUE_ARRAY) {
        return track(std::make_shared<ErrorValue>(
...
Value *rt_member_access(Value *obj, const char *name) {
    ...
    if (object->get_type() == VALUE_INSTANCE) {
        return track(dynamic_cast<InstanceValue *>(object.get())
                         ->get_member(name, object));
    }
    return track(std::make_shared<ErrorValue>(
        VALUE_ERROR,
        object->get_num() + " has no member " + std::string(name) + "\n"));
}
